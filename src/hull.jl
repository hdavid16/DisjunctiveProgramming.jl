# function hull_reformulation!(constr, bin_var, eps, i, j, k)
...
function hull_reformulation!(constr::ConstraintRef{<:AbstractModel, CI{SAF{T},V}}, bin_var, eps, i, j, k) where {T,V}
    linear_perspective_function(constr, bin_var, i)
end
function hull_reformulation!(constr, bin_var, eps, i, j, k)
    eps = get_reform_param(eps, i, j, k)
    nonlinear_perspective_function(constr, bin_var, eps, i)
end
...
function nonlinear_perspective_function(ref, bin_var, eps, i)
    ...
    @assert !isinf(g0.val) "Hull reformulation has failed for non-linear constraint $ref: $gx is not defined at 0. Perspective function is undetermined."
    g2 = FSG2*g0
